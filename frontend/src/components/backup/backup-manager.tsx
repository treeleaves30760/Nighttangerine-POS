...
export function BackupManager() {
	...
	const handleExport = async () => {
		try {
			...
			setMessage({
				type: "success",
				text: "Database backup exported successfully!",
			});
		} catch (error) {
			setMessage({
				type: "error",
				text: "Failed to export backup: " + (error as Error).message,
			});
				...
				<CardContent className="space-y-6">
						...
						<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
							<div className="text-center p-4 bg-muted/50 rounded-lg">
								<div className="text-2xl font-bold">
									{backupInfo.database.orders}
								</div>
								<div className="text-sm text-muted-foreground">Orders</div>
							</div>
							<div className="text-center p-4 bg-muted/50 rounded-lg">
								<div className="text-2xl font-bold">
									{backupInfo.database.products}
								</div>
								<div className="text-sm text-muted-foreground">Products</div>
							</div>
							<div className="text-center p-4 bg-muted/50 rounded-lg">
								<div className="text-2xl font-bold">
									{backupInfo.database.orderItems}
								</div>
					...
					<div className="space-y-3">
						...
						<p className="text-sm text-muted-foreground">
							Download a complete backup of your database including all orders,
					...
					<div className="space-y-3">
						...
						<p className="text-sm text-muted-foreground">
							Restore your database from a backup file. This will update
							existing records and create new ones as needed.
						</p>
